...
<%
	...
	if (sid == null)
		...
		try {
			...
			int Class = Integer.parseInt(request.getParameter("T2"));
			String sub = request.getParameter("T1");
			String subID;
			Subject z = em.createQuery(
					"SELECT c FROM Subject c WHERE c.Name='" + sub
							+ "'", Subject.class).getSingleResult();
			subID = z.getId();
			ArrayList<String> idList=(ArrayList<String>)em.createQuery("SELECT s.SID FROM Student s WHERE s.Class="+ Class).getResultList();
			for(String id:idList){
				ArrayList<Takes> temp;
				temp = (ArrayList<Takes>) em
						.createQuery(
								"SELECT c  FROM Takes c WHERE  c.subID='"
										+ subID
										+ "' AND  c.sid='"+id+"'").getResultList();
							...
							for (Takes t : resultList) {
								...
								if (month.equals("Annual")) {
									Iterator<String> itr = g.getGrades().keySet()
											.iterator();
									while (itr.hasNext()) {
										obtained += g.getGrades().get(itr.next())
												.intValue();
									}
